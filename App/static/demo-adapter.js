...
(function () {
    ...
    var PALETTE = [
        ...
        { index:  5, code: 'OAT', name: 'Oatmeal',      hex: '#D8CBB0', family: 'Neutral' },
        { index:  6, code: 'SLV', name: 'Silver',       hex: '#C9CDD2', family: 'Neutral' },
        { index:  7, code: 'LGY', name: 'Light Grey',   hex: '#AAB0B6', family: 'Neutral' },
        { index:  8, code: 'GRY', name: 'Grey',         hex: '#8A9099', family: 'Neutral' },
        { index:  9, code: 'STL', name: 'Steel',        hex: '#6E747C', family: 'Neutral' },
        { index: 10, code: 'CHL', name: 'Charcoal',     hex: '#3E4247', family: 'Neutral' },
        { index: 11, code: 'BLK', name: 'Black',        hex: '#1A1A1A', family: 'Neutral' },
        // Reds & Pinks
        { index: 12, code: 'CHY', name: 'Cherry',       hex: '#C1272D', family: 'Red / Pink' },
        { index: 13, code: 'RED', name: 'Red',          hex: '#E01B22', family: 'Red / Pink' },
        { index: 14, code: 'BRG', name: 'Burgundy',     hex: '#6E1E2A', family: 'Red / Pink' },
        { index: 15, code: 'COR', name: 'Coral',        hex: '#F2664F', family: 'Red / Pink' },
        { index: 16, code: 'WML', name: 'Watermelon',   hex: '#F04E6E', family: 'Red / Pink' },
        { index: 17, code: 'ROS', name: 'Rose',         hex: '#E68FA6', family: 'Red / Pink' },
        { index: 18, code: 'PNK', name: 'Pink',         hex: '#F6C6D4', family: 'Red / Pink' },
        { index: 19, code: 'HPK', name: 'Hot Pink',     hex: '#E93E97', family: 'Red / Pink' },
        { index: 20, code: 'ORC', name: 'Orchid',       hex: '#C86FB0', family: 'Red / Pink' },
        // Oranges, Yellows & Browns
        { index: 21, code: 'PMP', name: 'Pumpkin',      hex: '#E8722A', family: 'Orange / Yellow / Brown' },
        { index: 22, code: 'ORG', name: 'Orange',       hex: '#F5921B', family: 'Orange / Yellow / Brown' },
        { index: 23, code: 'GLD', name: 'Gold',         hex: '#F2B705', family: 'Orange / Yellow / Brown' },
        { index: 24, code: 'YEL', name: 'Yellow',       hex: '#F6D915', family: 'Orange / Yellow / Brown' },
        { index: 25, code: 'CRN', name: 'Cornsilk',     hex: '#F3E79A', family: 'Orange / Yellow / Brown' },
        { index: 26, code: 'CML', name: 'Camel',        hex: '#C79A5B', family: 'Orange / Yellow / Brown' },
        { index: 27, code: 'BRN', name: 'Brown',        hex: '#8A5A2B', family: 'Orange / Yellow / Brown' },
        { index: 28, code: 'COF', name: 'Coffee',       hex: '#5A3B22', family: 'Orange / Yellow / Brown' },
        { index: 29, code: 'CHO', name: 'Chocolate',    hex: '#3B2A20', family: 'Orange / Yellow / Brown' },
        // Greens
        { index: 30, code: 'LIM', name: 'Lime',         hex: '#8DC63F', family: 'Green' },
        { index: 31, code: 'SPR', name: 'Spring Green', hex: '#4FB24A', family: 'Green' },
        { index: 32, code: 'KEL', name: 'Kelly Green',  hex: '#2E9E4B', family: 'Green' },
        ...
        { index: 40, code: 'TRQ', name: 'Turquoise',   hex: '#17A2B8', family: 'Blue' },
        { index: 41, code: 'SKY', name: 'Sky',          hex: '#7EC8E3', family: 'Blue' },
        { index: 42, code: 'LBL', name: 'Light Blue',  hex: '#A9CCE3', family: 'Blue' },
        { index: 43, code: 'CFL', name: 'Cornflower',  hex: '#5B8DEF', family: 'Blue' },
        ...
        // Purples
        { index: 48, code: 'LAV', name: 'Lavender',    hex: '#C8A2D6', family: 'Purple' },
        { index: 49, code: 'AMY', name: 'Amethyst',    hex: '#9B59B6', family: 'Purple' },
        { index: 50, code: 'PUR', name: 'Purple',      hex: '#7A3EA1', family: 'Purple' },
    ...
    window.llmAPIManager = {
        ...
        requiresKey:   function () { return false; },
        hasServerKey:  function () { return false; },
        getKey:        function () { return ''; },
        setKey:        function () {},
        isLocal:       function () { return false; },
        getHeaders:    function () { return { 'Content-Type': 'application/json' }; },
        categorizeError: function (m) { return m || 'Error'; },
    };
    ...
    function handleConfig() {
        return resp({
            ...
            grid: {
                ...
                default_width: 60, default_height: 80,
                default_max_colors: 16, max_colors: 52,
            },
            upload: {
                max_file_size_mb: 25,
    ...
    function handleAnalyzeImage(init) {
        ...
        if (!(fd instanceof FormData)) { return resp({ error: 'No image data' }, 400); }

        var file     = fd.get('image');
        var reqW     = Math.max(5, Math.min(200, parseInt(fd.get('width')      || '60', 10)));
        var reqH     = Math.max(5, Math.min(200, parseInt(fd.get('height')     || '80', 10)));
        var maxClrs  = Math.max(2, Math.min(52,  parseInt(fd.get('max_colors') || '16', 10)));

        if (!file) { return resp({ error: 'No image file' }, 400); }
                ...
                try {
                    ...
                    for (var r = 0; r < reqH; r++) {
                        ...
                        for (var c = 0; c < reqW; c++) {
                            ...
                            if (pix[o + 3] < 128) { row.push(-1); continue; }
                            var key = (pix[o] << 16) | (pix[o + 1] << 8) | pix[o + 2];
                            if (!(key in cache)) { cache[key] = nearestPalIdx(pix[o], pix[o + 1], pix[o + 2]); }
                            row.push(cache[key]);
                        }
                        rawGrid.push(row);
                    }
                    ...
                    rawGrid.forEach(function (row) {
                        row.forEach(function (v) { if (v >= 0) { counts[v] = (counts[v] || 0) + 1; } });
                    });
                    ...
                    if (sorted.length > maxClrs) {
                        var allowed = {};
                        sorted.slice(0, maxClrs).forEach(function (i) { allowed[i] = true; });

                        // Remap over-limit colours to nearest allowed colour
                        var remap = {};
                        sorted.slice(maxClrs).forEach(function (ri) {
                            var bestD = Infinity, bestI = sorted[0];
                            sorted.slice(0, maxClrs).forEach(function (ai) {
                                var ar = PAL_RGB[ri], br = PAL_RGB[ai];
                                var d = (ar[0]-br[0])*(ar[0]-br[0]) +
                                        (ar[1]-br[1])*(ar[1]-br[1]) +
                                        (ar[2]-br[2])*(ar[2]-br[2]);
                    ...
                    resolve(new Response(JSON.stringify({
                        width: reqW, height: reqH, grid: finalGrid,
    ...
    function projectMeta(p) {
        ...
        (p.grid || []).forEach(function (row) {
            row.forEach(function (v) { if (v >= 0) { colors[v] = true; } });
        });
        return {
            id:          p.id,
            title:       p.title,
    ...
    function handleSaveProject(init) {
        ...
        var id    = 'proj_' + Date.now() + '_' + Math.random().toString(36).slice(2, 6);
        var proj  = {
            id: id, title: body.title || 'Untitled',
            grid: grid, width: w, height: h, updated_at: Date.now(),
        };
        var store = loadStore();
        store[id] = proj;
        saveStore(store);
    ...
    function handleLoadProject(id) {
        var store = loadStore();
        var p     = store[id];
        if (!p) { return resp({ error: 'Project not found' }, 404); }
        return resp({ project: p });
    }

    function handleUpdateProject(id, init) {
        var store = loadStore();
        if (!store[id]) { return resp({ error: 'Project not found' }, 404); }
        var body  = JSON.parse((init && init.body) || '{}');
    ...
    window.fetch = function (input, init) {
        ...
        if (url === '/api/config')         { return handleConfig(); }
        if (url === '/api/analyze-image')  { return handleAnalyzeImage(init); }

        if (url === '/api/projects') {
            var method = ((init && init.method) || 'GET').toUpperCase();
            if (method === 'GET')  { return handleGetProjects(); }
            if (method === 'POST') { return handleSaveProject(init); }
        }
        ...
        if (projM) {
            ...
            if (m === 'GET')    { return handleLoadProject(pid); }
            if (m === 'PUT')    { return handleUpdateProject(pid, init); }
            if (m === 'DELETE') { return handleDeleteProject(pid); }
        }
