...
(function () {
    'use strict';
    if (window.LLMAPIManager) { return; }
    ...
    class LLMAPIManager {
        ...
        setServerKeys(info) { this.serverKeys = info || {}; }
        ...
        hasServerKey(provider) {
            return !!(this.serverKeys[provider] && this.serverKeys[provider].available);
        }
        ...
        getKey(provider) { return this._all()[provider] || ''; }

        setKey(provider, key) {
            var all = this._all();
            if (key) { all[provider] = key; } else { delete all[provider]; }
        ...
        getHeaders(provider) {
            ...
            if (this.isLocal(provider)) { return headers; }
        ...
        categorizeError(message) {
            ...
            if (m.indexOf('connection') !== -1 || m.indexOf('unreachable') !== -1 || m.indexOf('refused') !== -1) {
                return 'Cannot reach the LLM server. Is it running?';
            }
            if (m.indexOf('api key') !== -1 || m.indexOf('authentication') !== -1 || m.indexOf('401') !== -1) {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        if (!window.llmAPIManager) { window.llmAPIManager = new LLMAPIManager(); }
    });
    if (!window.llmAPIManager) { window.llmAPIManager = new LLMAPIManager(); }
})();
