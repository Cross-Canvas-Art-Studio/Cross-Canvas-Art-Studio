...
(function () {
    ...
    class CrossStitchCanvas {
        ...
        setPalette(paletteArray) {
            this.paletteByIndex = {};
            (paletteArray || []).forEach(function (entry) {
        ...
        _recount() {
            ...
            for (var i = 0; i < this.cells.length; i++) {
                var v = this.cells[i];
                if (v >= 0) { this.counts[v] = (this.counts[v] || 0) + 1; }
            }
        }
        ...
        getDesign() {
            ...
            for (var r = 0; r < this.height; r++) {
                var row = [];
                for (var c = 0; c < this.width; c++) { row.push(this.cells[r * this.width + c]); }
                grid.push(row);
            }
        ...
        stitchCount() {
            var total = 0;
            for (var k in this.counts) { total += this.counts[k]; }
            return total;
        }
        colorCount() { return Object.keys(this.counts).length; }

        // ---- rendering ----
        fitToView() {
            var wrap = this.canvas.parentElement;
            if (!wrap || !this.width) { this.render(); return; }
            var availW = wrap.clientWidth - 20;
            var availH = Math.max(wrap.clientHeight - 20, 320);
            var size = Math.floor(Math.min(availW / this.width, availH / this.height));
            this.cellSize = Math.max(this.minCell, Math.min(this.maxCell, size || this.minCell));
            this.render();
        }
        ...
        zoomPercent() { return Math.round((this.cellSize / 14) * 100); }
        ...
        render() {
            ...
            for (var r = 0; r < this.height; r++) {
                for (var c = 0; c < this.width; c++) { this._paintCell(r, c, false); }
            }
        }
        ...
        _paintCell(r, c, clearFirst) {
            ...
            if (idx >= 0) {
                ...
                if (this.showSymbols && s >= 14) {
                    ctx.fillStyle = luminance(hex) > 0.55 ? 'rgba(0,0,0,0.75)' : 'rgba(255,255,255,0.85)';
        ...
        _drawStitch(x, y, s, hex) {
            var ctx = this.ctx;
            var inset = s * 0.16;
            var x0 = x + inset, y0 = y + inset, x1 = x + s - inset, y1 = y + s - inset;
            ctx.lineCap = 'round';
            ctx.lineWidth = Math.max(1.2, s * 0.30);
        ...
        setSelected(index) { this.selectedIndex = index; }
        ...
        _applyCell(r, c, value) {
            ...
            if (prev >= 0) { this.counts[prev]--; if (this.counts[prev] <= 0) delete this.counts[prev]; }
            this.cells[i] = value;
            if (value >= 0) { this.counts[value] = (this.counts[value] || 0) + 1; }
        ...
        _paintAt(e) {
            ...
            this._lastCell = key;
            if (this._applyCell(cell.r, cell.c, this._paintValue)) { this._notify(); }
        }
            ...
            for (var r = 0; r < this.height; r++) {
                for (var c = 0; c < this.width; c++) {
                    ...
                    if (idx >= 0) {
                        ...
                        var inset = s * 0.16;
                        octx.lineCap = 'round';
                        octx.lineWidth = Math.max(1.4, s * 0.30);
                        octx.strokeStyle = shade(hex, -22);
                        octx.beginPath(); octx.moveTo(x + inset, y + s - inset); octx.lineTo(x + s - inset, y + inset); octx.stroke();
                        octx.strokeStyle = hex;
                        octx.beginPath(); octx.moveTo(x + inset, y + inset); octx.lineTo(x + s - inset, y + s - inset); octx.stroke();
                    }
