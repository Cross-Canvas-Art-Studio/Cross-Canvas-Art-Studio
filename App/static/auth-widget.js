...
(function () {
    ...
    function field(type, ac, placeholder) {
        return h('input', { type: type, class: 'auth-input', autocomplete: ac || 'off', placeholder: placeholder || '' });
    }
    ...
    function buildModal() {
        ...
        if (required && guestEnabled) {
                    ...
                    _fetch('/auth/guest', { method: 'POST', headers: { 'Content-Type': 'application/json' } })
                        ...
                        .then(function (res) {
                            if (!res.ok) {
                                ...
                                guestBtn.disabled = false; guestBtn.textContent = 'Continue as Guest';
                                return;
                            }
            ...
            guestSection = h('div', { class: 'auth-guest-section' }, [
                h('p', { class: 'auth-guest-note' }, ['Guest sessions last up to 24 hours and are not saved permanently. For persistent projects, create an account.']),
                guestBtn
            ]);
        }

        var intro = required
            ? h('p', { class: 'auth-intro auth-intro-required', text: 'Sign in to use Cross Canvas Art.' })
            : h('p', { class: 'auth-intro', text: 'Sign in to save and own your projects.' });

        var bodyParts = [intro];
        if (registrationEnabled) { bodyParts.push(h('div', { class: 'auth-tabs' }, [tabSignIn, tabRegister])); }
        ...
        if (!required) { headerKids.push(h('button', { class: 'icon-btn', title: 'Close', type: 'button', onclick: closeModal }, ['✕'])); }
        var hdr = h('div', { class: 'app-modal-header' }, headerKids);
        var dialog = h('div', { class: 'app-modal-dialog auth-dialog' }, [hdr, body]);
        modalEl = h('div', { class: 'app-modal', id: 'authModal', 'aria-hidden': 'true', role: 'dialog' }, [dialog]);
    ...
    function buildGate(registrationEnabled) {
        ...
        gateEl = h('div', { class: 'auth-gate', id: 'authGate', 'aria-live': 'polite' }, [
            h('div', { class: 'auth-gate-card' }, [
                ...
                h('button', { class: 'btn-primary auth-gate-open', type: 'button', text: gateButtonText, onclick: openModal })
            ])
        ]);
