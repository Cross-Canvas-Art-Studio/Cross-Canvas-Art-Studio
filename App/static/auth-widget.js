/**
 * Auth Widget - optional account UI for Cross Canvas Art.
 *
 * Self-contained vanilla JS. When the backend reports auth is enabled it:
 *  - transparently attaches the stored bearer token to /api and /auth requests,
 *  - renders a sign-in chip / account menu in the header,
 *  - shows a login + register (and optional guest) modal,
 *  - and, when auth is REQUIRED, gates the whole page until the user signs in.
 * Ported from the Live Translate project.
 */
(function () {
  "use strict";

  var LOCAL_KEY = "cca_auth_token";
  var SESSION_KEY = "cca_auth_token_s";
  ...
  // Attach bearer token to same-origin API/auth requests.
  ...
  function labeled(label, input) {
    return h("label", { class: "auth-field" }, [
      h("span", { class: "auth-label", text: label }),
      input,
    ]);
  }
  ...
  function submit(path, payload, remember) {
    ...
    _fetch(path, {
      ...
      .then(function (res) {
        if (!res.ok) {
          showError(res.d.error || "Request failed");
          return;
        }
  ...
  function passkeyRegister(username, email) {
    return _fetch("/auth/passkey/register/options", {
      ...
      .then(function (res) {
        if (!res.ok) {
          throw new Error(res.d.error || "Options request failed");
        }
        var cid = res.d.cid;
        return navigator.credentials
          .create({ publicKey: _toCreationOpts(res.d.options) })
          .then(function (cred) {
          ...
          .then(function (res) {
            if (!res.ok) {
              throw new Error(res.d.error || "Verification failed");
            }
            return res.d;
          });
      });
  ...
  function passkeyLogin() {
    return _fetch("/auth/passkey/login/options", {
      ...
      .then(function (res) {
        ...
        var cid = res.d.cid;
        return navigator.credentials
          .get({ publicKey: _toRequestOpts(res.d.options) })
          .then(function (cred) {
          ...
          .then(function (res) {
            if (!res.ok) {
              throw new Error(res.d.error || "Authentication failed");
            }
            return res.d;
          });
      });
  ...
  function buildModal() {
    ...
    var signInForm = h(
      "form",
      {
        ...
        onsubmit: function (e) {
          ...
          submit(
            "/auth/login",
            { username: siUser.value.trim(), password: siPass.value },
            siRemember.checked,
          );
        },
      },
      [
        ...
        h("button", { type: "submit", class: "btn-primary auth-submit" }, [
          "Sign In",
        ]),
      ],
    );
      ...
      if (registrationEnabled) {
        var pkRegBtn = h(
          "button",
          {
            ...
            onclick: function () {
              ...
              passkeyRegister(uname, rEmail.value.trim())
                .then(function (res) {
                  onAuthenticated(res.token, true);
                })
                .catch(function (err) {
        ...
        passkeyRegSection = h("div", { class: "auth-passkey-section" }, [
          ...
          h("p", { class: "auth-guest-note" }, [
            "Use the username (and optional email) above, then confirm with your device biometrics or PIN. No password needed.",
          ]),
          pkRegBtn,
        ]);
    ...
    modalSetTab = function (name) {
      ...
      if (passkeySignInSection)
        passkeySignInSection.style.display = reg ? "none" : "";
      if (passkeyRegSection)
        passkeyRegSection.style.display = reg ? "" : "none";
      showError("");
    };
    ...
    if (required && guestEnabled) {
      ...
      guestSection = h("div", { class: "auth-guest-section" }, [
        h("p", { class: "auth-guest-note" }, [
          "Guest sessions last up to 24 hours and are not saved permanently. For persistent projects, create an account.",
        ]),
        guestBtn,
      ]);
    }
    ...
    var intro = required
      ? h("p", {
          class: "auth-intro auth-intro-required",
          text: "Sign in to use Cross Canvas Art.",
        })
      : h("p", {
          class: "auth-intro",
          text: "Sign in to save and own your projects.",
        });

    var bodyParts = [intro];
    if (registrationEnabled) {
      bodyParts.push(
        h("div", { class: "auth-tabs" }, [tabSignIn, tabRegister]),
      );
    }
    ...
    var headerKids = [h("h2", { text: "Cross Canvas Art — Account" })];
    if (!required) {
      headerKids.push(
        h(
          "button",
    ...
    var dialog = h("div", { class: "app-modal-dialog auth-dialog" }, [
      hdr,
      body,
    ]);
    modalEl = h(
    ...
    modalEl.addEventListener("click", function (e) {
      if (!required && e.target === modalEl) closeModal();
    });
    document.addEventListener("keydown", function (e) {
      if (!required && e.key === "Escape") closeModal();
    });
  }
    ...
    if (!token()) {
      headerEl.appendChild(
        h(
          "button",
          {
            class: "btn-secondary auth-signin-btn",
            type: "button",
            onclick: function () {
              if (modalSetTab) modalSetTab("signin");
              openModal();
            },
          },
          ["Sign in"],
  ...
  function renderUserChip(user) {
    ...
    menu.appendChild(
      h(
        "button",
        {
          class: "auth-menu-item auth-menu-danger",
          type: "button",
          onclick: logout,
        },
        ["Log out"],
    ...
    var chip = h(
      "button",
      {
        class: "btn-secondary auth-chip" + (isGuest ? " auth-chip-guest" : ""),
        type: "button",
        onclick: function (e) {
          e.stopPropagation();
  ...
  function buildGate(registrationEnabled) {
    ...
    gateEl = h(
      "div",
      { class: "auth-gate", id: "authGate", "aria-live": "polite" },
      [
        h("div", { class: "auth-gate-card" }, [
          h("div", { class: "auth-gate-logo" }, [h("span", { text: "CC" })]),
          h("h1", { class: "auth-gate-title", text: "Cross Canvas Art" }),
          h("p", { class: "auth-gate-sub", text: "Sign in to get started" }),
          h("button", {
            class: "btn-primary auth-gate-open",
            type: "button",
            text: gateButtonText,
            onclick: openModal,
          }),
