...
Uses itsdangerous (bundled with Flask) for signed, time-limited tokens, plus the
stdlib `secrets` module for cryptographically-secure random codes and
constant-time comparisons. Fernet (from cryptography) is used to encrypt stored
API keys at rest.
...
_serializer = URLSafeTimedSerializer(_SECRETS_KEY, salt='cross-canvas-art.v1')
