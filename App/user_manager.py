...
revocation for the optional authentication system.

Storage lives in a single SQLite database under /data so it persists on the same
volume as projects. bcrypt is used for password hashing. All access uses
short-lived per-operation connections (WAL mode) which is safe for the app's
low-concurrency auth workload.
...
def purge_expired_guests():
    """Delete guest users whose TTL has elapsed and their settings."""
    with _connect() as conn:
        rows = conn.execute(
            'SELECT user_id FROM users WHERE is_guest = 1 AND expires_at IS NOT NULL AND expires_at < ?',
            (int(time.time()),)
        ).fetchall()
    ...
    with _write_lock, _connect() as conn:
        ...
        conn.commit()
    logger.info('Purged %d expired guest accounts', len(ids))
    return len(ids)
